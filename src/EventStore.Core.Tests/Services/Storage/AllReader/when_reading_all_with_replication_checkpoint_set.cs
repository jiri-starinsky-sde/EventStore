...
namespace EventStore.Core.Tests.Services.Storage.AllReader
{
        ...
        : ReadIndexTestScenario
    {
        ...
        long _nonReplicatedEventPosition;
        ...
        protected override void WriteTestScenario()
        {
            ...
            commit = WriteCommit(res.LogPosition, "ES2", 1);
            _nonReplicatedEventPosition = commit.LogPosition;
        ...
        [Test]
        public void should_return_same_event_number_as_next_event_number_when_reading_stream_forward_from_an_event_after_replication_checkpoint()
        {
            var result = ReadIndex.ReadStreamEventsForward("ES2", 1, 10);
            Assert.AreEqual(0, result.Records.Length, "Number of records returned");
            Assert.AreEqual(1, result.NextEventNumber, "Next event number");
        }

        [Test]
        public void should_return_same_position_as_next_position_when_reading_all_forward_from_a_position_replication_checkpoint()
        {
            var fromPos = new TFPos(_nonReplicatedEventPosition, _nonReplicatedEventPosition);
            var result = ReadIndex.ReadAllEventsForward(fromPos, 10);
            Assert.AreEqual(0, result.Records.Count, "Number of records returned");
            Assert.AreEqual(fromPos, result.NextPos, "Next position");
        }
