...
namespace EventStore.Core.Tests.ClientAPI
{
    ...
    public class catch_up_subscription_to_stream_handles_non_replicated_events
    {
        private static readonly int TimeoutMs = 2000;
        private static readonly string StreamId = "catch_up_subscription_handles_non_replicated_events";
        private const int _expectedReads = 5;
        private const int _lastEventNumber = 1;
        ...
        public void SetUp()
        {
            ...
            _subscription = new EventStoreStreamCatchUpSubscription(_connection, new ConsoleLogger(), StreamId, null, null,
                (subscription, ev) =>
                {
            ...
            _connection.HandleReadStreamEventsForwardAsync((stream, start, max) =>
                {
                    ...
                    var read = new StreamEventsSlice(SliceReadStatus.Success, stream, start,
                        ReadDirection.Forward, events.ToArray(), nextEventNumber, _lastEventNumber, true);
                    _readCount++;
            ...
            _connection.HandleSubscribeToStreamAsync((stream, raise, drop) =>
            {
                ...
                var sub = new VolatileEventStoreSubscription(
                    new VolatileSubscriptionOperation(new NoopLogger(),
                        ...
                        StreamId, false, null, raise, drop, false, () => null), StreamId, -1, _lastEventNumber);
        ...
        private ClientMessage.ResolvedIndexedEvent CreateEvent(long eventNumber)
        {
            ...
            return new ClientMessage.ResolvedIndexedEvent(evnt, evnt);
        }
